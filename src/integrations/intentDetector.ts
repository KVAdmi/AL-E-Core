...
export type ExternalDataIntent = 
  | { type: 'NONE' }
  | { type: 'EXCHANGE_RATE_USD_MXN'; query: string }
  | { type: 'WEATHER'; query: string; city?: string }
  | { type: 'NEWS'; query: string; topic?: string }
  | { type: 'CRYPTO_PRICE'; query: string; symbol?: string };
 ...
 */
export function detectExternalDataIntent(message: string): ExternalDataIntent {
  ...
  if (cryptoPatterns.some(pattern => pattern.test(text))) {
    ...
    if (/\bbitcoin\b|\bbtc\b/i.test(text)) symbol = 'BTC';
    else if (/\bethereum\b|\beth\b/i.test(text)) symbol = 'ETH';
